...
To access the admin view of the database, login with **admin@eshop.de** and **admin**. After that click **Add new server**. Enter a name and switch to the **connection** tab. There you need to check the IP address of the postgres container with the command `docker inspect postgresdb`, at the end of the command output you will find the **IPAddress** that is used as the Hostaddress. Enter the username **root** and password **root** and click **Save** to log in.   
